/*************************************************************
*	파 일 명  : StdPrcsCon.java
*	작성일자  : 2004/06/22
*	작 성 자  : 
*	내    용  : 표준공정관리 제어(CRUD)처리 클래스
*************************************************************/
...
import java.util.*;
public class StdPrcsCon extends WmsServlet{
	...
	protected void performTask(HttpServletRequest req, HttpServletResponse res)	throws Exception {
		try {
			...
			if(_ACT.equals("I")){//사용안함
				...
				dto.set(req);
				dto.remove("empId");
				dto.remove("cdType"); //표준공정에서 cdType을 input type으로 사용하므로 제거함
				dto.remove("stdPrcsType"); //표준공정에서 cdType을 input type으로 사용하므로 제거함
				dto.remove("dupYN");
				dto.remove("key1");
				dto.remove("key2");
				dto.remove("value1");
