	...
	<td>
		<table width=100% height=24 cellpadding=0 cellspacing=1 class=tablecalendar>
			<tr valign=middle class=movecalendar>
				<td width=22 style="vertical-align:bottom" align=left>&nbsp;<a href="/common/calendar.jsp?field=<%=fieldname%>&min=<%=mindate%>&max=<%=maxdate%>&base=<%=preMonth%><%=(idx!=null)?"&idx="+idx:""%>"><img src="../images/b_schd_arrowleft.gif" border=0 width=13 height=11  alt="지난달로 가기"></a></td>
...
%>
				...
				<td width=22 style="vertical-align:bottom" align=right><a href="/common/calendar.jsp?field=<%=fieldname%>&min=<%=mindate%>&max=<%=maxdate%>&base=<%=nextMonth%><%=(idx!=null)?"&idx="+idx:""%>"><img src="../images/b_schd_arrowright.gif" border=0 width=13 height=11 alt="다음달로 가기"></a>&nbsp;</td>
			</tr>
			</table>
	</td>
<tr align=center>
		...
		<table width=100% height=150 cellspacing=0 cellpadding=0 border=0>
		   ...
		   for(int i=0;i<42;i++){%>
			...
			<%=(i%7==0)?"<tr  bgcolor=white align=center>":""%>
		          <td><a href="javascript:ReturnDate<%=(idx!=null&&!idx.trim().equals(""))?"2":""%>('<%=cdate[i]%>','<%=mindate%>','<%=maxdate%>','<%=fieldname%>'<%=(idx!=null&&!idx.trim().equals(""))?","+idx:" "%>)"
...
<tr>
	<td>
		<table border=0 cellpadding=0 cellspacing=1 width=100%><tr><td><span id=LUNDate><span>
			&nbsp;</td><td width=60 align=center><A href="/common/calendar.jsp?field=<%=fieldname%>&min=<%=mindate%>&max=<%=maxdate%>&base=<%=currentdate%><%=(idx!=null)?"&idx="+idx:""%>">
