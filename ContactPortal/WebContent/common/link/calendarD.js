...
function RefreshCalendar(tfield, base, max, min, idx){
	...
	document.location.href = "/common/calendarD.jsp?field=" + tfield +"&min="+ min + "&max="+ max +"&base="+ base +"&idx="+idx;
}
...
function RefreshCalendar2(){
	...
	idx2 = document.all.MonthList.selectedIndex
	base = document.all.YearList.options[idx1].value + "-"+ document.all.MonthList.options[idx2].value +"-01";
	document.location.href = "/common/calendarD.jsp?field=" + fieldname +"&min="+ mindate + "&max="+ maxdate +"&base="+ base+onlineMenu2;
}
...
function openCalendarD(tfield, base, max, min) {
     CalPath =  "/common/calendarD.jsp?field=" + tfield + "&base=" + base + "&max=" + max + "&min=" + min;
     openW=window.open(CalPath,"","width=200,Height=220,location=center,left=300,top=200");
}
//tfield가 반복되는 경우 index를 입력받아 처리하는 함수
function openCalendarD2(tfield, base, max, min, idx) {
     CalPath =  "/common/calendarD.jsp?field=" + tfield + "&base=" + base + "&max=" + max + "&min=" + min +"&idx="+idx;
...
function ReturnDateD(tnew,tmin,tmax,tfield) {
	...
	if (tmax == "") {
		if (tnew >= tmin) {
			...
			self.opener.setDate(tnew,tmin,tmax,tfield);
...
function ReturnDateD2(tnew,tmin,tmax,tfield,idx) {
	if (tmax == "") {
		if (tnew >= tmin) {
