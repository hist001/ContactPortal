...
function jobChange(s){//직무관련  선택시
	...
	if(s!='RC')
	document.forms[0].action='/common/histJobSearch.jsp'
...
function allSearch(){//제품,직무판단
	s="/common/histJobSearch.jsp";
    if (document.forms[0].rdo[0].checked) {
		s="/common/WorkItemProdSearch.jsp";
	}
	openWin(s, "", '600', '500');   	
...
function selectItem(prodNo,prodName,prcsNo,prcsName,realStartDt,realEndDt,mh){//parent에 값전달	
	if(document.forms[0].url.value=='createWorkPlanDetail'){
		...
		//수주관리 공정 체크 기간안에서만 등록가능
		var curPrcs=prcsNo.substring(1);
		if(curPrcs=='B0'||curPrcs=='D0'||curPrcs=='F0'){
			today = new Date();
			var todate=today.getFullYear()+"";
			todate+=(today.getMonth()>8)?(today.getMonth()+1)+"":"0"+(today.getMonth()+1);
			todate+=(today.getDate()>9)?today.getDate()+"":"0"+today.getDate();
			//alert(todate);
			if(realStartDt>todate||realEndDt<todate){
				alert('수주제품은 해당 공정기간 내에서만 보고할 수 있습니다.');
				return;
			}
		}
		//수주관리 끝
...
function rdoJob_click() {//직무선택
	...
	document.forms[0]._SCREEN.value="/common/empItemSearch.jsp"
	document.forms[0].action='/commonCon';
	document.forms[0].method='post'
	//"/commonCon?_ACT=RE&_SCREEN=/common/empItemSearch.jsp&empId=<%=user.empId%>"
	document.forms[0].submit();
...
function rdochk1_click() {//직무,제품선택 - 계약이전이후 처리
//alert("aaa");
    document.all["rowa"].style.display="";
    document.all["rowb"].style.display="none";
}

function rdochk2_click() {//직무,제품선택 - 계약이전이후 처리
    document.all["rowa"].style.display="none";
    document.all["rowb"].style.display="";
}

//20041020 허대영 추가 - 제품에서 청구수금관리를 위한 function
function rdochk3_click() {//제품 - 청구수금관리 OPEN
//alert("aaa");
    document.all["rowc"].style.display="";
    document.all["rowd"].style.display="none";
}

function rdochk4_click() {//제품 - 청구수금관리 CLOSE
    document.all["rowc"].style.display="none";
    document.all["rowd"].style.display="";
}
