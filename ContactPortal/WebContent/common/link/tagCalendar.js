...
function getMonthMetrixHtml(_year, _month) {
    ...
    for(i=0; i < 6; i++) {
        ...
        for(j = 0; j < 7; j++) {
            ...
            _metrixHtml += "<td height='23' bgcolor='"+bgColor+"' borderColor='#FFFFFF' valign='middle' align='center' style='cursor:hand;' onclick=\"parent.setDateExit('"+_strDate+"');\">";
...
function getCalendarHtml(_year, _month) {
    var _html = "";
    _html += ("<table border='0' cellspacing='4' cellpadding='0' width='100%' bgcolor='#88A8D8'>");//달력 테두리 색상
    _html += ("<tr>");
    ...
    _html += ("&nbsp;<SELECT name='tbSelYear' onChange='parent.resetDate(tbSelYear.value, tbSelMonth.value)' style='font-family:돋움; font-size:13px; color:#737373;'>");
    ...
    _html += ("</SELECT>");
    _html += ("&nbsp;<select name='tbSelMonth' onChange='parent.resetDate(tbSelYear.value, tbSelMonth.value)' style='font-family:돋움; font-size:13px; color:#737373;'>");
...
*******************************************************************/
function CalendarStart(textField, txtIndex, startYear, endYear, seper) {
...
function getcal(param) {
 	 CalendarStart(param, 0, 2000, null, '-');
}
