...
function jobChange(s){//직무관련  선택시
	...
	if(s!='RC')
	document.forms[0].action='/Budget/histJobSearch.jsp'
...
function itemSearch(){//제품검색
	...
	document.forms[0]._SCREEN.value='/Budget/WorkItemSearch.jsp'
...
function allSearch(){//제품,직무판단
	s="/Budget/histJobSearch.jsp";
    if (document.forms[0].rdo[0].checked) {
		s="/Budget/WorkItemProdSearch.jsp";
	}
	openWin(s, "", '600', '500');   	
...
function selectItem(prodNo,prodName,prcsNo,prcsName,realStartDt,realEndDt,mh,idx){//parent에 값전달	
	if(document.forms[0].url.value=='createWorkPlanDetail'){
		...
		opener.selectItem(prodNo,prodName,prcsNo,prcsName,realStartDt,realEndDt,mh,idx);
		self.close();
	}

}

function select(jobNo,jobName,bizNo,bizName,idx){//parent에 값 전달
	...
	if(document.forms[0].url.value=='createWorkPlanDetail'){
		...
		opener.setValue(jobNo,jobName,bizNo,bizName,idx);
		self.close();
	}
}
...
function rdoJob_click() {//직무선택
	...
	document.forms[0]._SCREEN.value="/Budget/empItemSearch.jsp"
	document.forms[0].action='/commonCon';
	document.forms[0].method='post'
	//"/commonCon?_ACT=RE&_SCREEN=/common/empItemSearch.jsp&empId=<%=user.empId%>"
	document.forms[0].submit();
