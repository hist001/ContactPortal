...
function sendDataObj(code,sObj,sObjNo,sObjCd){
	...
	if(sObjNo != undefined)sObjNo.value=objCodes[1];
	if(sObjCd != undefined)sObjCd.value=objCodes[0];
...
function rmData(){
	...
	try{
		if(iText.length>0) {
			for(i=0;iText.length>0;i++)	iText[0].removeNode(true);
		}else{
...
function viewData(obj){
	...
	if(obj!=''&& obj.value!=''){
		autoComidx=0;	
		gfn_ajax_service(obj.name,new Array(obj.value),'comPopup','COM_001','message');
	}
}
//자동검색
function autoSearch(obj,left,top,objNo,objCd){
	(left == undefined)?left=0:left;
	(top == undefined )?top=0:top;
	
//	autoSrarchObj=document.getElementById('autoSearch');
//	autoSrarchObj.style.display='';
	
	if(objNo != undefined )	sObjNo=document.getElementById(objNo);
	if(objCd != undefined )	sObjCd=document.getElementById(objCd);
...
function autoObjXYSet(source,target,left,top){
	...
	targetObj.style.top=ret.top-top;
}
/**
 * 좌표 값을 측정한다.
 * @param obj 측정 요망 객체
