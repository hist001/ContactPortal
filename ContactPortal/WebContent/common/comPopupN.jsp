...
function confirmItem(codeNo,code,name,code2,name2,objcodeNo,objcode,objcodeName,objcode2,objcodeName2){
	 ...
	 var objcodeNo_o= frm.getElementsByTagName(objcodeNo) ;
	 
    if (objcodeNo!= null && objcodeNo!="" && objcodeNo!='undefined'){
       if (frm.all(objcodeNo).length==undefined){
	    	frm.all(objcodeNo).value = codeNo;
	    	frm.all(objcodeNo).style.color="blue";
    	}else{
    	 	frm.all(objcodeNo)[<%=idx%>].value = codeNo;
	    	frm.all(objcodeNo)[<%=idx%>].style.color="blue";
    	}    	
	}
	
    if (objcode!= null & objcode!="" & objcode!='undefined') {
       if (frm.all(objcode).length==undefined){
	    	frm.all(objcode).value = code;
	    	frm.all(objcode).style.color="blue";
    	}else{
	    	frm.all(objcode)[<%=idx%>].value= code;
	    	frm.all(objcode)[<%=idx%>].style.color="blue";
    	}
    }
    if (objcodeName!= null & objcodeName!="" & objcodeName!='undefined') {
       if (frm.all(objcodeName).length==undefined){
	    	frm.all(objcodeName).value = name;
	    	frm.all(objcodeName).style.color="blue";
    	}else{
	    	frm.all(objcodeName)[<%=idx%>].value= name;
	    	frm.all(objcodeName)[<%=idx%>].style.color="blue";
    	}
    }
    if (objcode2!= null & objcode2!="" & objcode2!='undefined') {
       if (frm.all(objcode2).length==undefined){
	    	frm.all(objcode2).value = code2;
	    	frm.all(objcode2).style.color="blue";
    	}else{
	    	frm.all(objcode2)[<%=idx%>].value= code2;
	    	frm.all(objcode2)[<%=idx%>].style.color="blue";
    	}
    }
    if (objcodeName2!= null & objcodeName2!="" & objcodeName2!='undefined') {
       if (frm.all(objcodeName2).length==undefined){
	    	...
	    	frm.all(objcodeName2).style.color="blue";
    	}else{
	    	frm.all(objcodeName2)[<%=idx%>].value= name2;
	    	frm.all(objcodeName2)[<%=idx%>].style.color="blue";
    	}    	
    }
...
function search(){
	...
	frm._SCREEN.value = '/common/comPopupN.jsp?empid='+frm.empId.value+'&code='+ frm.code.value + '&codeName=' + frm.codeName.value+'&setYmd=' + frm.setYmd.value; 
...
function init(){
    ...
    //코드 사전 입력 시 또는 초기 조회 처리 (auteSel : 'A')
...
<Table cellpadding="0" cellspacing="0" border="0"	width="100%" id="table1">
		...
		<tr id="<%=dtos[i].level%>_<%=dtos[i].highCode%>"   bordercolor=#0066CC
			...
			<td width="" height="24" align="left">
				...
				<%if (code.equals("") & codeName.equals("") ||param.equals("emp") ||paramId.equals("treeView")){ %>			
					<%=Utility.makeTree(dtos,i)%>
				<%}else{ %>
