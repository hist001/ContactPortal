...
<%	//검색화면에서 테이블명에 따른 코드이름 표기
	...
	if(radioSel==null)  radioSel="job_radio";
    ...
    if(dto != null){
    	dtoParam2 =Utility.parseInt(dto[0].param2); 
    }
%>
...
function confirmItem(codeNo,code,name,code2,name2){
    ...
    if(frm.param.value=='job_new'){ //직무 선택 후 조직 조회
	   ...
	   if(frm.param.value=='pjt_new' & frm.radioSel[1].checked){ 
		    ...
		    if (codeNo!= null && codeNo!="" && codeNo!='undefined'){
			     if (opener.document.all('<%=codeNo1%>').length==undefined){
					...
					opener.document.all('<%=actFlag%>').value='D';
			   		...
			   		opener.document.all('<%=codeNo1%>')[<%=idx%>].value=codeNo;
					opener.document.all('<%=codeCd1%>')[<%=idx%>].value=code;
					opener.document.all('<%=codeName1%>')[<%=idx%>].value=name;
					opener.document.all('<%=codeView1%>')[<%=idx%>].value=code+" "+name;
					opener.document.all('<%=actFlag%>')[<%=idx%>].value='D';
	    		} 
	    	}
	    ...
	    if (codeNo!= null && codeNo!="" && codeNo!='undefined'){
	     if (opener.document.all('<%=codeNo1%>').length==undefined){
			...
			opener.document.all('<%=actFlag%>').value='I';
		   	...
		   	opener.document.all('<%=codeNo1%>')[<%=idx%>].value=frm.jobCd.value;
			opener.document.all('<%=codeCd1%>')[<%=idx%>].value=frm.jobCd.value;
			opener.document.all('<%=codeName1%>')[<%=idx%>].value=frm.jobName.value;
			opener.document.all('<%=codeView1%>')[<%=idx%>].value=frm.jobCd.value+" "+frm.jobName.value;
	
			opener.document.all('<%=codeNo2%>')[<%=idx%>].value=codeNo;
			opener.document.all('<%=codeCd2%>')[<%=idx%>].value=code;
			opener.document.all('<%=codeName2%>')[<%=idx%>].value=name;
			opener.document.all('<%=codeView2%>')[<%=idx%>].value=code+" "+name;
			opener.document.all('<%=actFlag%>'[<%=idx%>]).value='I';
    		}	 
    	}
		self.close();
	   }
...
function search(){
	...
	if (frm.radioSel[0].checked){		//직무+조직
	...
	frm._ACT.value = 'AC';	
	frm._SCREEN.value = '/common/comPopup3.jsp?empid='+frm.empId.value
											+'&code='+ frm.code.value 
											+'&CHK1='+ frm.CHK1.value 
...
<Table cellpadding="0" cellspacing="0" border="0"	width="100%" id="table1">
		...
		<tr id="<%=dtos[i].level%>_<%=dtos[i].highCode%>"  bordercolor="#0066CC"
			class='onbase'
			style="cursor:hand;<%if (dtos[i].level> dtoParam2){ out.print("display:none");}%>" 
