...
<Table  cellpadding=3 cellspacing=1 border=0 class=table1 width=100% id="table1">
	<tr>
		<td width="20%" class=TD1>사업코드</td>
		<td width="80%" class=TD1>사업코드명</td>
	</tr>
</Table>
<Table  cellpadding=3 cellspacing=1 border=0 class=table1 width=100% id="table1">
<%for(int i=0;i<prodList.length;i++){%>
	<tr class='base' style ="cursor:hand;" onMouseOver="this.className='on'" onMouseOut="this.className='base'" onclick="javascript:selectItem('<%=prodList[i].prodNo%>','<%=prodList[i].prodName%>','<%=prodList[i].bizNo%>','<%=prodList[i].bizAcqCd%>','<%=prodList[i].contDt%>','<%=prodList[i].deliDt%>','<%=prodList[i].goalMh%>'
)">
		...
		<td width="35%" class='viewContentL'><%=prodList[i].bizAcqCd%></td>
	</tr>
<%}%>	
</table>
<%}%>
